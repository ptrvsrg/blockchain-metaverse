...
public class CompilationDeploying {
    ...
    public Hash160 deployMapContract(ContractParameter parameter, String name) throws Throwable {
        ...
        try {
            mapContractHash = deployContract(MapChangesContract.class.getCanonicalName(), parameter, replaceMap);
     ...
     * @throws Throwable если происходит ошибка при развертывании
     */
    public Hash160 deployStateContract(ContractParameter parameter, String name) throws Throwable {
        HashMap<String, String> replaceMap = new HashMap<>(1);
        replaceMap.put("Name", name);

        Hash160 stateContractHash;
        try {
            stateContractHash = deployContract(PlayerPositionContract.class.getCanonicalName(), parameter, replaceMap);
        } catch (TransactionConfigurationException e) {
            if (e.getMessage().contains("Contract Already Exists: ")) {
     ...
     * @throws Throwable если происходит ошибка при развертывании
     */
    private Hash160 deployContract(String contractClassName, ContractParameter parameter, Map<String, String> replaceMap) throws Throwable {
        CompilationUnit res;
